...
const Requests = ({ User, initialRequests, pendingRequestCount, initialPage, totalPages }) => {
  ...
  useEffect(() => {
    const fetchRequests = async (page) => {
      try {
        const res = await fetch(`http://localhost:4000/v1/requests?page=${page}`);
  ...
  const handleAcceptRequest = async (requestId) => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/v1/requests/${requestId}/accept`, {
        method: 'POST',
        headers: {
      ...
      setRequestStatuses((prevStatuses) => ({ ...prevStatuses, [requestId]: 'accepted' }));
  ...
  const handleRejectRequest = async (requestId) => {
    try {
        const response = await fetch(`http://localhost:4000/v1/requests/${requestId}/reject`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        setRequestStatuses((prevStatuses) => ({ ...prevStatuses, [requestId]: 'rejected' }));
  ...
  return (
    <div>
      ...
      <BaseLayout>
        ...
        <SectionHeader title={`Pending Cinefellow Requests (${pendingReqCount})`} />
          {requests.map((request) => (
            ...
            <CinefellowRequestCard
              ...
              requestedAt={new Date(request.created_at).toLocaleString('en-US', {
                month: 'long', // "January"
                day: '2-digit', // "01"
                year: 'numeric', // "2024"
                hour: '2-digit', // "12"
...
export async function getServerSideProps(context) {
  ...
  try{
    ...
    const { pendingRequests, pendingRequestCount } = await fetchData(`http://localhost:4000/v1/requests?page=${page}&limit=${limit}`);
