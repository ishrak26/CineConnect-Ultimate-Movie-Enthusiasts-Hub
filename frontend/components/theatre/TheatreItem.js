...
function TheaterItem({Theater,showDir=false}) {
    // const GOOGLE_API_KEY=process.env.NEXT_PUBLIC_GOOGLE_API_KEY;
    const photo_ref=Theater?.photos?Theater?.photos[0]?.photo_reference:''
    // const {userLocation,setUserLocation}=useContext(UserLocationContext);
    const [userLocation,setUserLocation]=useState({lat:40.7128,lng:-74.0060})//[40.7128, -74.0060
    ...
    const onDirectionClick=()=>{
      ...
      +','+Theater.geometry.location.lng+'&travelmode=driving')
  }
    ...
    <div className='w-[195px] flex-shrink-0 p-2
     ...
     bg-white hover:scale-110 transition-all mt-[20px] cursor-pointer'>
        ...
        />}
         <h2 className='text-[13px] font-bold mt-1 line-clamp-1'>{Theater.name}</h2>
                <h2 className='text-[10px] text-gray-400 
                line-clamp-2'>{Theater.formatted_address}</h2>
                ...
                <h2 className='text-[10px] font-bold'>{Theater.rating}</h2>
            </div>
