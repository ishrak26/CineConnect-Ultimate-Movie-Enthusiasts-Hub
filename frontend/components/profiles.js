      ...
      <BaseLayout>
        <div className="container">
          <main className="flex-grow p-4">
            <div className="user-profile-details flex items-end mb-16 mt-8">
              {' '}
              {/* mb-8 adds margin-bottom */}
              <img
                src={userData.profilePic}
                alt={`${userData.name}'s profile`}
                className="rounded-full h-40 w-40 mr-8"
              />
              ...
              <button className="text-sm text-blue-500 hover:text-blue-700 self-start">
                Edit profile
              </button>{' '}
              {/* self-start aligns to the top */}
            </div>
...
export async function getServerSideProps(context) {
  ...
  try {
    const [watched, watchlist, reviews, forums, cinefellows] =
      await Promise.all([
        fetchData(`${process.env.NEXT_PUBLIC_SERVER_URL}/v1/movies/`), // dummy movie cards, will need to fetch actual watched movies by the user
        fetchData(`${process.env.NEXT_PUBLIC_SERVER_URL}/v1/movies/`), // dummy movie cards, will need to fetch actual watchlist movies by the user
        ...
        fetchData(`${process.env.NEXT_PUBLIC_SERVER_URL}/v1/movies/`), // dummy, will need to change to fetch cinefellows
      ])
